...
module.exports = exports = function mx(schema, opts) {
	...
	.static('findByNameAndPassword', function(name, password, cb) {
		this.findOne({name: name}, function(err, obj) {
			if(err) return cb.apply(obj, arguments);
...
MX.prototype = {
	...
	load: function(req, res, next) {
		...
		if(req.session[accessor].isRegistered) {
			this.Model.findOne(user._id, function(err, dbUser) {
				if(err)
					return next(err);

				this._pubObj(req, dbUser || user, dbUser);
				return next();
			})
		}
		else {
			this._pubObj(req, user, false);
			return next();
		}
	}
}
